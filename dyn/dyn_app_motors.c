...
void canviar_velocitat(uint8_t module_id, uint16_t speed, byte direction){
	byte mov_speed_l, mov_speed_h;
	mov_speed_l= speed & 0xFF; //Associem a l'mov_speed_l els primers 8 bytes de speed value
	mov_speed_h=( (direction << 2) & 0x04) | ((speed >> 8) & 0x03); //Associem a l'mov_speed_h els 2 bytes que falten de speed value i la direcci�
	uint8_t parameters[3];
	parameters[0]= DYN_REG__MOV_SPEED__L;
	parameters[1]=mov_speed_l; //Posem a mov_speed_l el Moving Speed(L)
	parameters[2]=mov_speed_h; //Posem a mov_speed_h el Moving Speed(H)
